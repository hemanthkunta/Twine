...
import { Reply, Smile, Pin, Edit3, Trash2, Globe, Play, Pause, Download, Volume2, Shield, Bot, MessageSquare, Eye, BarChart2 } from 'lucide-react';
...
const renderFormattedText = (text: string) => {
  ...
  return lines.map((line, lIdx) => {
    ...
    return (
      <div key={lIdx} className={line.trim().startsWith('•') || /^\d+\./.test(line.trim()) ? 'my-0.5' : 'my-0'}>
        {parts.map((part, pIdx) => {
          ...
          if (part.startsWith('@')) {
            return (
              <span key={pIdx} className="font-medium text-[#3fc5f0] hover:underline cursor-pointer">
                {part}
              </span>
            );
          }
      ...
      <div className="w-full max-w-3xl px-4 py-4 space-y-3 flex-1 flex flex-col min-h-full">
          ...
          <div className="sticky top-2 z-20 w-full mb-3 p-2.5 px-4 rounded-2xl bg-[#17212b]/95 backdrop-blur-md border border-[rgba(255,255,255,0.08)] flex items-center justify-between shadow-lg">
            <div className="flex items-center space-x-2.5 min-w-0">
              ...
              <div className="text-xs min-w-0">
                ...
                <div className="text-white/80 truncate">{pinnedMessage.content_text}</div>
              </div>
            </div>
            <button
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center my-auto text-center space-y-2 opacity-60">
            ...
            <p className="text-xs text-[#7f91a4]">Send a message to start the conversation!</p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    {isHovered && (
                      ...
                      >
                        ...
                        <button
                          onClick={() => handleTranslate(msg.id, msg.content_text)}
                    ...
                    >
                      {!isOut && (
                        <div className="text-xs font-semibold text-[#3fc5f0] mb-1 flex items-center space-x-1.5">
                          <span>{msg.sender?.display_name || 'User'}</span>
                          {msg.sender?.is_bot && (
                      ...
                      {msg.reply_to && (
                        <div className="mb-2 pl-2.5 py-1 border-l-2 border-[#3fc5f0] bg-black/25 rounded text-xs">
                          <div className="font-semibold text-[#3fc5f0]">{msg.reply_to.sender_name}</div>
                          <div className="text-white/80 truncate text-[11px]">{msg.reply_to.content_text}</div>
                        </div>
                      )}
                        ...
                        <div className="my-1 min-w-[240px] space-y-1.5">
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1 flex items-center space-x-1 h-6">
                              {(msg.media_metadata?.waveform || [0.4, 0.7, 0.9, 0.5, 0.8, 0.4, 0.9, 0.6, 0.3]).map(
                                (bar: number, i: number) => (
                                  <div
                                    key={i}
                          ...
                          <div className="p-2 rounded-xl bg-black/25 border border-white/10 text-xs text-white/90">
                            <div className="flex items-center justify-between text-[10px] text-[#3fc5f0] font-semibold mb-1">
                              ...
                              <span className="text-[9px] text-emerald-400 font-mono">96% confidence</span>
                            </div>
                      ...
                      {msg.type === 'IMAGE' && msg.media_url && (
                        <div className="my-1 rounded-xl overflow-hidden max-w-sm">
                          <img src={msg.media_url} alt="Attachment" className="w-full h-auto object-cover rounded-xl" />
                        </div>
                      )}

                      {/* Interactive Community Poll / Quiz */}
                      {(msg.type === 'POLL' || msg.poll) && msg.poll && (
                        <PollCard poll={msg.poll} currentUserId={currentUser.id} />
                      )}
                      ...
                      {onOpenThread && (
                        <div className="mt-1.5 pt-1 flex items-center justify-between border-t border-white/5 pr-14">
                          ...
                          >
                            ...
                            <span>
                              {msg.thread_message_count && msg.thread_message_count > 0
                      ...
                      <div className="absolute right-2.5 bottom-1.5 flex items-center space-x-1 text-[10px] text-white/60 select-none">
                        ...
                        {msg.transport_mode === 'MESH' && (
                          <span className="text-[9px] px-1 py-0.2 bg-purple-500/25 text-purple-300 rounded font-mono font-medium flex items-center space-x-0.5">
                            <span>📡</span>
                            <span>{msg.hop_count === 1 ? '1 hop' : `${msg.hop_count || 1} hops`}</span>
                          </span>
                        )}
                        {msg.status === 'QUEUED' && (
                          <span className="text-[9px] text-amber-300 font-medium">⏳ Queued</span>
                        )}
                        {msg.is_edited && <span className="text-[9px]">edited</span>}
                        <span>{formatMessageTime(msg.created_at)}</span>
                        {isOut && <StatusTicks status={msg.status} isSending={msg.isSending} />}
                      </div>
