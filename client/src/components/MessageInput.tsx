...
import { Send, Smile, Paperclip, Mic, X, Image, FileText, MapPin, Sparkles, Check, Edit3 } from 'lucide-react';
...
interface MessageInputProps {
  ...
  onClearReply: () => void;
  onClearEdit: () => void;
  onSendMessage: (text: string, replyToId?: string, mediaUrl?: string, mediaType?: any, mediaMetadata?: any) => void;
...
export const MessageInput: React.FC<MessageInputProps> = ({
  ...
  const handleSendVoiceNote = async (audioBase64: string, waveform: number[], duration: number) => {
      ...
      <div className="w-full max-w-3xl relative">
        ...
        {replyToMessage && (
          <div className="flex items-center justify-between px-3.5 py-2 mb-2 bg-[#17212b] border-l-2 border-[#3fc5f0] rounded-r-xl border border-[rgba(255,255,255,0.06)] shadow-sm">
            <div className="text-xs min-w-0">
              ...
              <div className="text-[#7f91a4] truncate">{replyToMessage.content_text}</div>
            </div>
            <button onClick={onClearReply} className="p-1 text-[#7f91a4] hover:text-white rounded-lg">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        {editingMessage && (
          <div className="flex items-center justify-between px-3.5 py-2 mb-2 bg-[#17212b] border-l-2 border-amber-400 rounded-r-xl border border-[rgba(255,255,255,0.06)] shadow-sm">
            ...
            <button onClick={onClearEdit} className="p-1 text-[#7f91a4] hover:text-white rounded-lg">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
        ...
        {showEmojiPicker && (
          <div className="absolute bottom-16 left-2 sm:left-4 z-30 p-3 bg-[#1e2a38] border border-[rgba(255,255,255,0.1)] rounded-2xl shadow-2xl flex flex-wrap gap-2 max-w-[calc(100vw-2rem)] w-72 animate-fade-in">
        ...
        {/* Attachment Popover */}
        {showAttachMenu && (
          <div className="absolute bottom-16 left-2 sm:left-12 z-30 p-2 bg-[#1e2a38] border border-[rgba(255,255,255,0.1)] rounded-2xl shadow-2xl space-y-1 w-52 max-w-[calc(100vw-2rem)] animate-fade-in">
            <button
        ...
        {isVoiceRecording ? (
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2 bg-[#17212b] border border-[rgba(255,255,255,0.08)] rounded-2xl p-1 sm:p-1.5 shadow-lg">
            <button
              onClick={() => {
              ...
              className={`p-2.5 rounded-xl transition-all ${
                showEmojiPicker ? 'text-[#3fc5f0] bg-[#242f3d]' : 'text-[#7f91a4] hover:text-white hover:bg-[#242f3d]'
              }`}
              title="Emojis"
            >
            ...
            <button
              ...
              className={`p-2.5 rounded-xl transition-all ${
                showAttachMenu ? 'text-[#3fc5f0] bg-[#242f3d]' : 'text-[#7f91a4] hover:text-white hover:bg-[#242f3d]'
              }`}
            ...
            <input
              ...
              placeholder={editingMessage ? 'Edit your message...' : isUploading ? 'Uploading media...' : 'Write a message... (type @ai to query bot)'}
