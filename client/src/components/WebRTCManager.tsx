...
export const WebRTCManager: React.FC<WebRTCManagerProps> = ({
    ...
    // Every hardware media track acquired during this call.
    // This guarantees camera/microphone resources are released even
    // when a track came from a temporary getUserMedia() stream.
    const acquiredMediaTracksRef = useRef<Set<MediaStreamTrack>>(new Set());

    const registerMediaStream = (stream: MediaStream | null) => {
        if (!stream) return;

        stream.getTracks().forEach((track) => {
            acquiredMediaTracksRef.current.add(track);
        });
    };

    const stopAllAcquiredMediaTracks = () => {
        acquiredMediaTracksRef.current.forEach((track) => {
            try {
                track.enabled = false;
                ...
                console.warn('[WebRTC] Failed to stop media track:', err);
            }
        });

        acquiredMediaTracksRef.current.clear();
    };
    ...
    const lastBytesReceivedRef = useRef<{ bytes: number; timestamp: number }>({
        bytes: 0,
        timestamp: 0,
    });
    ...
    const resumeAudioContext = () => {
        ...
        if (
            persistentAudioElRef.current &&
            persistentAudioElRef.current.paused &&
    ...
    const attachRemoteMedia = useCallback(() => {
        ...
        if (localVideoRef.current) {
            const activeLocalStream = isScreenSharing
        ...
        connectionWatchdogTimerRef.current = setTimeout(async () => {
            if (pcRef.current && callStatus !== 'connected' && !hasEndedRef.current) {
                if (isIceRestartingRef.current) {
                    console.log(
                        '[WebRTC Watchdog] ICE restart already in flight. Skipping overlapping cycle.'
                    );
                    return;
                }

                console.warn(
                    `[WebRTC Watchdog] Connection timeout after ${timeoutMs}ms. Attempting ICE recovery...`
                );
                if (iceRestartAttemptsRef.current < 2) {
                    ...
                    setErrorMessage(
                        'Unable to establish peer connection. Please verify network access.'
                    );
                    sounds.stopDialTone();
                    sounds.stopRingtone();
                    sounds.playCallEnd();
    ...
    const restartIce = async () => {
        ...
        if (pc.signalingState !== 'stable') {
            console.log(
                '[WebRTC ICE Restart] Waiting for signaling state to become stable before restart.'
            );
            return;
        }
    ...
    const createPeerConnection = (
        ...
        if (videoTransceiver) {
            ...
            pc.addTransceiver('video', {
                direction: callType === 'video' ? 'sendrecv' : 'recvonly',
            });
        }
        ...
        console.log(
            `[WebRTC Timeline] [${new Date().toISOString()}] Initializing RTCPeerConnection...`
        );
        ...
        pc.onconnectionstatechange = () => {
            console.log(
                `[WebRTC] PeerConnection State: ${pc.connectionState}, ICE Connection State: ${pc.iceConnectionState}`
            );
            if (pc.connectionState === 'connected') {
                ...
                console.warn('[WebRTC] PeerConnection closed. Ending call session.');
                cleanup();
                onEndCall(durationRef.current);
            } else if (pc.connectionState === 'failed' || pc.connectionState === 'disconnected') {
                setCallStatus('reconnecting');
                armConnectionWatchdog(5000);
            }
        };
        ...
        pc.ontrack = (event) => {
            console.log(
                `[WebRTC ontrack] Track received: kind=${event.track.kind}, id=${event.track.id}, muted=${event.track.muted}, readyState=${event.track.readyState}, callStatus=${callStatus}`
            );
            ...
            if (event.track.kind === 'audio') {
                ...
                event.track.onunmute = () => {
                    console.log(
                        '[WebRTC ontrack] 🔊 Remote audio track unmuted (RTP packet stream active)! Attaching audio.'
                    );
                    attachRemoteMedia();
                };
                event.track.onmute = () => {
                    console.log('[WebRTC ontrack] Remote audio track muted temporarily.');
                };
            }

            if (event.track.kind === 'video') {
                console.log(
                    `[WebRTC ontrack] 📹 Remote video track registered: id=${event.track.id}, muted=${event.track.muted}`
                );
                ...
                event.track.onunmute = () => {
                    console.log(
                        '[WebRTC ontrack] 📹 Remote video track unmuted! Refreshing media sinks.'
                    );
                    setRemoteHasVideo(true);
                    attachRemoteMedia();
                };
    ...
    const getLocalMediaStream = async (): Promise<MediaStream> => {
        try {
            ...
            console.error(
                '[CRITICAL] getUserMedia failed, using SYNTHETIC fallback audio/video instead of real mic/camera:',
                err?.name,
                err?.message,
                err
            );
            setFallbackNotice(
                `Microphone/Camera hardware access failed (${err?.name || 'Permission Denied'})`
            );
            ...
            try {
                ...
                const canvasStream = canvas.captureStream
                    ...
                    : (canvas as any).mozCaptureStream?.(15);
                if (canvasStream) {
    ...
    const startOutgoingCall = async () => {
        try {
            ...
            console.error(
                '[WebRTC] Error starting outgoing call / TURN credential fetch failed:',
                err
            );
            setCallStatus('failed');
            setErrorMessage(
    ...
    const toggleScreenShare = async () => {
        try {
            if (!isScreenSharing) {
                ...
                registerMediaStream(screenStream);
                ...
                if (pc) {
                    ...
                    try {
                        ...
                        console.warn(
                            '[WebRTC] Renegotiation offer creation error on screen share:',
                            renErr
                        );
                    } finally {
                        isMakingOfferRef.current = false;
    ...
    const cleanup = () => {
        ...
        // 0. GUARANTEED HARDWARE RELEASE
        stopAllAcquiredMediaTracks();
    ...
    useEffect(() => {
        ...
        } else if (!outgoingCallStartedRef.current) {
            outgoingCallStartedRef.current = true;
        ...
        const unsubRenegotiate = wsClient.on('webrtc:renegotiate', async (payload) => {
            ...
            if (payload.offer && pc) {
                try {
                    const sdpLines = payload.offer.sdp?.split('\n') || [];
                    const videoIndex = sdpLines.findIndex((l: string) => l.startsWith('m=video'));
                    const videoLine = videoIndex !== -1 ? sdpLines[videoIndex] : 'none';
                    const videoDirectionLine =
                        videoIndex !== -1
                            ? sdpLines
                                  .slice(videoIndex, videoIndex + 10)
                                  .find(
                                      (l: string) =>
                            ...
                            : 'none';
                    console.log(
                        `[WebRTC Renegotiate] Received Offer SDP Video Info: m-line="${videoLine?.trim()}", direction="${videoDirectionLine?.trim()}"`
                    );
                    ...
                    if (offerCollision && isPolite) {
                        console.log(
                            '[WebRTC Glare] Polite peer rolling back to accept colliding offer'
                        );
                        await Promise.all([
                    ...
                    // Configure answering video transceiver direction appropriately
                    const videoTransceiver = findVideoTransceiver();
                    if (videoTransceiver) {
                        videoTransceiver.direction = isVideoActiveRef.current
                            ? 'sendrecv'
                            : 'recvonly';
                    }
                    ...
                    const answerVideoIndex = answerLines.findIndex((l: string) =>
                        l.startsWith('m=video')
                    );
                    const answerVideoDir =
                        answerVideoIndex !== -1
                            ? answerLines
                                  .slice(answerVideoIndex, answerVideoIndex + 10)
                                  .find(
                                      (l: string) =>
                                          l.startsWith('a=sendrecv') ||
                                          l.startsWith('a=recvonly') ||
                            ...
                            : 'none';
                    console.log(
                        `[WebRTC Renegotiate] Created Answer SDP Video Direction: "${answerVideoDir?.trim()}"`
                    );
                    ...
                    if (
                        payload.is_video_active ||
                        (videoTransceiver?.receiver?.track &&
                            videoTransceiver.receiver.track.readyState === 'live')
                    ) {
                        setRemoteHasVideo(true);
                    }
                } catch (renErr) {
            ...
            if (payload.answer && pc) {
                try {
                    ...
                    const vTransceiver = findVideoTransceiver();
                    console.log(
                        `[WebRTC Renegotiate] Answer processed. Video Transceiver: direction=${vTransceiver?.direction}, currentDirection=${vTransceiver?.currentDirection}`
                    );
                } catch (ansErr) {
                    console.error(
                        '[WebRTC] Error setting remote description on renegotiation answer:',
                        ansErr
                    );
                }
            }
    ...
    const showVideoView =
        isVideoActive || remoteHasVideo || isScreenSharing || remoteIsScreenSharing;
            ...
            {callStatus === 'incoming_ringing' ? (
                <div className="w-full max-w-sm bg-gradient-to-b from-[#1e2c3a] to-[#0f1720] sm:rounded-3xl rounded-2xl overflow-hidden shadow-2xl border border-white/15 flex flex-col items-center justify-between p-6 sm:p-8 text-center min-h-[380px] sm:min-h-[440px] animate-in fade-in zoom-in duration-200 m-4">
                    <div className="space-y-2">
                        ...
                        <h3 className="text-xl font-black text-white mt-2 truncate max-w-[280px]">
                            {peer.display_name}
                        </h3>
                        <p className="text-xs text-[#7f91a4] truncate">
                            @{peer.username} is calling you on Twine
                        </p>
                    </div>
                ...
                <div className="w-full h-full sm:h-auto sm:max-w-2xl bg-gradient-to-b from-[#182533]/95 via-[#0e1621]/98 to-[#090e14]/99 sm:rounded-3xl rounded-none shadow-[0_25px_60px_-15px_rgba(0,0,0,0.85)] border-0 sm:border sm:border-white/10 flex flex-col min-h-[100dvh] sm:min-h-[520px] max-h-[100dvh] sm:max-h-[85vh] justify-between overflow-hidden">
                            ...
                            {stats && callStatus === 'connected' && (
                                ...
                                >
                                    ...
                                    {stats.packetLossPercent > 0 && (
                                        <span>• {stats.packetLossPercent}%</span>
                                    )}
                                </button>
                            )}
                    ...
                    <div className="relative flex-1 flex items-center justify-center overflow-hidden min-h-[260px] sm:min-h-[340px]">
                        ...
                        {fallbackNotice && (
                            <div className="absolute top-16 left-4 right-4 z-30 p-3 bg-amber-500/20 border border-amber-500/40 rounded-xl text-amber-200 text-xs flex items-center space-x-2 backdrop-blur-md animate-pulse">
                                <AlertTriangle className="w-4 h-4 flex-shrink-0 text-amber-400" />
                                <span>
                                    <strong>Hardware Warning:</strong> {fallbackNotice}. Placeholder
                                    audio/video is active.
                                </span>
                            </div>
                        )}
                            ...
                            <div className="flex flex-col items-center justify-center space-y-5 py-8 px-4 w-full">
                                <div className="relative flex items-center justify-center">
                                    ...
                                    <UserAvatar
                                        ...
                                        avatarUrl={peer.avatar_url}
                                        size="xl"
                                        className="shadow-[0_10px_35px_rgba(0,0,0,0.6)] relative z-10 scale-105"
                                    />
                                </div>
                                <div className="text-center space-y-1.5 px-4 max-w-sm">
                                    <p className="text-lg sm:text-2xl font-bold text-white tracking-wide truncate">
                                        {peer.display_name}
                                    </p>
