...
export class ApiService {
  ...
  static init() {
    // Proactively refresh when page recovers from mobile background
  ...
  private static scheduleAutoRefresh() {
    ...
    // Proactively refresh access token every 10 minutes (15 min expiry)
  ...
  private static async request<T>(endpoint: string, options: RequestInit = {}, isRetry = false): Promise<T> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // On 401 Unauthorized: Attempt silent refresh and retry before clearing session
        if (res.status === 401 && !isAuthEndpoint && !isRetry) {
          ...
          if (refreshed) {
            ...
            console.warn('Silent refresh failed. Session expired or revoked on endpoint:', endpoint);
  ...
  static async refreshSession(): Promise<boolean> {
    ...
    // Mutex: If a refresh is already in progress, await it
  ...
  static async createDirectChat(userId: string): Promise<{ chat: Chat }> {
  ...
  static async updateGroup(groupId: string, params: {
    title?: string;
    description?: string;
    avatarUrl?: string;
    memberLimit?: number;
  ...
  static async getSmartReplies(chatId: string): Promise<{ suggestions: string[] }> {
    return this.request('/ai/smart-replies', {
      method: 'POST',
      body: JSON.stringify({ chatId }),
    });
  }
