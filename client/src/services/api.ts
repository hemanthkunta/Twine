...
export class ApiService {
    static getMediaUrl(mediaUrl: string): string {
        if (!mediaUrl) {
            return '';
        }

        // Already using the protected media endpoint.
        if (mediaUrl.startsWith('/api/media/')) {
            return mediaUrl;
        }

        // Convert old stored media URLs:
        // /uploads/filename.webm
        // ->
        // /api/media/filename.webm
        ...
        // Handle a complete URL pointing to the old uploads endpoint.
        try {
            ...
            if (url.pathname.startsWith('/uploads/')) {
                ...
                return `/api/media/${encodeURIComponent(decodeURIComponent(filename))}`;
            }
        } catch {
            // Not an absolute URL; return it unchanged below.
        }
