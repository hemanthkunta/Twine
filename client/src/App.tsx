...
import { Heart, Shield, Sparkles, Check, Search, Compass, Radio, ShieldAlert, Bookmark, Settings } from 'lucide-react';
import { User, UserSummary, Chat, Message, ReceiptStatus, TransportMode, MeshPacket } from './types/index';
...
export function App() {
  ...
  const [suggestedTopics, setSuggestedTopics] = useState<string[]>(['#WebRTC', '#Security', '#MeshRelay']);
  ...
  const triggerScreenshotWarning = () => {
    setScreenshotAlert('⚠️ Screenshot / Screen recording attempt detected in confidential chat.');
    sounds.playReceived();
  ...
  useEffect(() => {
    ...
    ApiService.getMessages(activeChatId)
      .then((res) => {
        ...
        offlineStorage.saveMessagesLocally(res.messages);

        // Fetch AI smart replies for the chat
        ApiService.getSmartReplies(activeChatId).then((replyRes) => {
          setSmartReplies(replyRes.replies || []);
        });

        // Mark messages as read
        ApiService.markChatRead(activeChatId).then((readRes) => {
          if (readRes.count > 0) {
  ...
  useEffect(() => {
    ...
    const unsubNewMsg = wsClient.on('chat:new_message', (payload: { message: Message; chat_id: string }) => {
    ...
    // C. Message Edited
    const unsubMsgEdited = wsClient.on('chat:message_edited', (payload: { message: Message; chat_id: string }) => {
      setMessages((prev) =>
    ...
    // D. Message Deleted
    const unsubMsgDeleted = wsClient.on('chat:message_deleted', (payload: { message_id: string; chat_id: string }) => {
      setMessages((prev) => prev.filter((m) => m.id !== payload.message_id));
    });

    // E. Reaction Updated
    const unsubReaction = wsClient.on('chat:reaction_updated', (payload: { message_id: string; chat_id: string; reactions: Record<string, string[]> }) => {
      setMessages((prev) =>
        prev.map((m) =>
    ...
    // F. Pinned Message
    const unsubPin = wsClient.on('chat:message_pinned', (payload: { chat_id: string; pinned_message: Message | null }) => {
      setChats((prev) =>
        prev.map((c) =>
          c.id === payload.chat_id ? { ...c, pinned_message: payload.pinned_message || undefined } : c
        )
      );
    });
    ...
    // H. Receipt updates
    const unsubReceipt = wsClient.on('chat:receipt_update', (payload) => {
      ...
      setChats((prev) =>
        prev.map((c) => {
          if (c.last_message && c.last_message.id === message_id) {
            return {
              ...c,
              last_message: { ...c.last_message, status: status as ReceiptStatus },
            };
          }
          return c;
        })
  ...
  const handleSendMessage = async (
    ...
    setChats((prev) =>
      prev.map((c) =>
        c.id === activeChatId
          ? { ...c, last_message: optimisticMessage, updated_at: optimisticMessage.created_at }
          : c
      )
    );
  };
  ...
  const handleEditSubmit = (messageId: string, newText: string) => {
    ...
    setMessages((prev) =>
      prev.map((m) => (m.id === messageId ? { ...m, content_text: newText, is_edited: true } : m))
    );
  };
    ...
    <div className="flex flex-col h-screen w-screen text-theme-primary bg-theme-primary overflow-hidden relative selection:bg-[#2f88ff]/30">
      ...
      {screenshotAlert && (
        <div className="sticky top-0 z-50 bg-gradient-to-r from-red-600 to-amber-600 px-4 py-2 flex items-center justify-between text-xs font-semibold shadow-xl animate-bounce">
          ...
          <button onClick={() => setScreenshotAlert(null)} className="text-white/80 hover:text-white">
            Dismiss
          </button>
        </div>
        )}
      ...
      <div className="flex flex-1 overflow-hidden relative z-10">
        {currentUser ? (
          <>
            <div className={`w-full md:w-80 lg:w-96 flex-shrink-0 h-full ${mobileChatOpen ? 'hidden md:flex' : 'flex'}`}>
              <ChatList
            ...
            {activeChat ? (
              <main className={`flex-1 flex flex-col h-full bg-theme-primary relative min-w-0 ${!mobileChatOpen ? 'hidden md:flex' : 'flex'}`}>
                <ChatHeader
                  chat={activeChat}
                  isOnline={activeChat.peer_user ? onlineUserIds.has(activeChat.peer_user.id) : false}
                  ...
                  onOpenChannelDashboard={() => setShowChannelDashboardModal(true)}
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-theme-primary">
                ...
                <p className="text-sm text-theme-secondary max-w-md mb-6 leading-relaxed">
                  Private real-time messenger built exclusively for couples and close friends with E2EE, WebRTC audio/video calls, and offline mesh resilience.
                </p>
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-theme-primary">
            ...
            <p className="text-xs text-[#7f91a4] mb-6">Connecting your private couple & friends vault...</p>
            <button
              onClick={() => {
                if (demoUsers.length > 0) {
                  ApiService.demoLogin(demoUsers[0].id).then(r => setCurrentUser(r.user));
                } else {
                  setShowAuthModal(true);
                }
              }}
