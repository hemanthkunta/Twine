...
export function initDatabase() {
  ...
  try { db.exec('ALTER TABLE users ADD COLUMN is_bot INTEGER DEFAULT 0'); } catch {}
  try { db.exec('ALTER TABLE messages ADD COLUMN is_pinned INTEGER DEFAULT 0'); } catch {}
...
function seedInitialData() {
  ...
  for (const u of demoUsers) {
    insertUserStmt.run(u.id, u.phone_number, u.username, u.display_name, u.bio, u.avatar_url, defaultPasswordHash);
  }

  // 2. Pre-seed Direct Chat (Alice & Bob)
  const directChatId = 'chat_alice_bob_101';
  db.prepare(`INSERT OR IGNORE INTO chats (id, type, title, is_e2ee) VALUES (?, 'DIRECT', ?, 0)`).run(
    directChatId,
    'Alice & Bob'
  );
  db.prepare(`INSERT OR IGNORE INTO chat_members (id, chat_id, user_id, role) VALUES (?, ?, ?, 'MEMBER')`).run(
    'cm_101_1',
    directChatId,
    'usr_alice_001'
  );
  ...
  db.prepare(`INSERT OR IGNORE INTO chat_members (id, chat_id, user_id, role) VALUES (?, ?, ?, 'ADMIN')`).run(
    'cm_ai_2',
    aiChatId,
    'usr_ai_bot'
  );
  ...
  `).run('msg_grp_001', groupId, 'usr_alice_001', 'Welcome to the Core Engineering supergroup! 🚀 We are deploying our WebSockets and WebRTC engine.');

  db.prepare(`
    INSERT OR IGNORE INTO messages (id, chat_id, sender_id, type, content_text, created_at)
    VALUES (?, ?, ?, 'TEXT', ?, datetime('now', '-18 minutes'))
  `).run('msg_grp_002', groupId, 'usr_bob_002', 'The multi-device sync and presence latency tests are looking stellar! Under 15ms fanout.');

  db.prepare(`
    INSERT OR IGNORE INTO messages (id, chat_id, sender_id, type, content_text, created_at)
    VALUES (?, ?, ?, 'TEXT', ?, datetime('now', '-15 minutes'))
  `).run('msg_grp_003', groupId, 'usr_charlie_003', 'I added support for voice notes with live audio waveforms 🎙️');
