...
async function runSecurityAudit() {
  ...
  const noneAlgToken = 'eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJpZCI6InVzcl9hbGljZV8wMDEiLCJpYXQiOjE2MDAwMDAwMDB9.';
  ...
  auditResults.push({
    ...
    notes: noneVerify === null ? 'Rejected unsigned token successfully' : 'VULNERABILITY: Accepted none algorithm token!',
  });

  // Test B: Tampered payload with original signature
  const tokenParts = demoAuth.token.split('.');
  const tamperedPayload = Buffer.from(JSON.stringify({ id: 'usr_admin_root', username: 'root' })).toString('base64url');
  ...
  auditResults.push({
    ...
    notes: tamperedVerify === null ? 'Cryptographic signature mismatch caught and rejected.' : 'VULNERABILITY: Accepted modified payload!',
  });
  ...
  for (const a of auditResults) {
    if (a.prevented) {
      ...
      vulnCount++;
      console.log(`  ⚠️ [VULNERABILITY] ${a.vulnerabilityCategory}: ${a.testCase} - ${a.notes}`);
    }
  }
