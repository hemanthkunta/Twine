...
export class ChatService {
    ...
    `
        ...
        ).run(cm1, chatId, userAId);
        db.prepare(
            `INSERT INTO chat_members (id, chat_id, user_id, role) VALUES (?, ?, ?, 'MEMBER')`
        ).run(cm2, chatId, userBId);
    ...
    static getChatById(chatId: string, userId: string): Chat | null {
      ...
      `
        ...
        const memberCountRow = db
            .prepare('SELECT COUNT(*) as count FROM chat_members WHERE chat_id = ?')
    ...
    `
        ...
        const lastMessage = lastMsgRow
            ? MessageService.getMessageById(lastMsgRow.id) || undefined
            : undefined;
    ...
    `
        ...
        // Pinned message
        const pinRow = db
            .prepare(`SELECT message_id FROM pinned_messages WHERE chat_id = ?`)
            .get(chatId) as { message_id: string } | undefined;
        const pinnedMessage = pinRow
            ? MessageService.getMessageById(pinRow.message_id) || undefined
            : undefined;
