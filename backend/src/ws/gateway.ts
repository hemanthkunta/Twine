    ...
    socket.on('message', async (raw: string) => {
      ...
      try {
        ...
        if (clientSession && !RateLimiter.checkWsLimit(clientSession.userId, 50)) {
          sendError('RATE_LIMIT_EXCEEDED', 'WebSocket frame rate limit exceeded (max 50 frames/sec)', correlation_id);
          return;
        }
        ...
        if (type === 'auth:handshake') {
          ...
          registeredClient = PresenceService.registerConnection(user.id, socket, authPayload.device_id);
        ...
        if (!clientSession) {
          sendError('UNAUTHENTICATED', 'Must complete auth:handshake first', correlation_id);
          return;
        }
        ...
        if (type === 'chat:send_message') {
          ...
          const hasAITrigger = message.content_text.trim().toLowerCase().startsWith('@ai');
          ...
          if (isDirectAIChat || hasAITrigger) {
            setTimeout(() => {
              ...
              setTimeout(() => {
                PresenceService.broadcastTyping(message.chat_id, 'usr_ai_bot', memberIds, false);
        ...
        if (type === 'chat:edit_message') {
          ...
          const updated = MessageService.editMessage(editPayload.message_id, senderId, editPayload.content_text);
          if (updated) {
            const memberIds = ChatService.getChatMemberIds(editPayload.chat_id);
            const frame: WSFrame = {
              type: 'chat:message_edited',
              payload: { message: updated, chat_id: editPayload.chat_id },
              timestamp: Date.now(),
            };
            PresenceService.broadcastToUsers(memberIds, frame, socket);
          }
          return;
        }
        ...
        if (type === 'chat:delete_message') {
          ...
          const { success, chatId } = MessageService.deleteMessage(delPayload.message_id, senderId);
          if (success) {
            const memberIds = ChatService.getChatMemberIds(chatId);
            const frame: WSFrame = {
              type: 'chat:message_deleted',
              payload: { message_id: delPayload.message_id, chat_id: chatId },
              timestamp: Date.now(),
            };
            PresenceService.broadcastToUsers(memberIds, frame, socket);
          }
        ...
        if (type === 'chat:react') {
          ...
          const { chatId, reactions } = MessageService.toggleReaction(reactPayload.message_id, senderId, reactPayload.emoji);
          const memberIds = ChatService.getChatMemberIds(chatId);
          const frame: WSFrame = {
            type: 'chat:reaction_updated',
            payload: { message_id: reactPayload.message_id, chat_id: chatId, reactions },
            timestamp: Date.now(),
          };
          PresenceService.broadcastToUsers(memberIds, frame, socket);
          return;
        ...
        if (type === 'chat:pin_message') {
          ...
          if (pinPayload.is_pinned) {
            GroupService.pinMessage(pinPayload.chat_id, pinPayload.message_id, senderId);
          } else {
            GroupService.unpinMessage(pinPayload.chat_id);
          }
          const memberIds = ChatService.getChatMemberIds(pinPayload.chat_id);
          const pinnedMsg = pinPayload.is_pinned ? MessageService.getMessageById(pinPayload.message_id) : null;
          const frame: WSFrame = {
        ...
        if (type === 'chat:typing') {
          ...
          if (!typingPayload.chat_id) return;
          const memberIds = ChatService.getChatMemberIds(typingPayload.chat_id);
          PresenceService.broadcastTyping(typingPayload.chat_id, senderId, memberIds, Boolean(typingPayload.is_typing));
          return;
        }
        ...
        if (type === 'chat:read_receipt') {
          ...
          if (!receiptPayload.chat_id || !receiptPayload.message_id) return;

          const { updated, chat_id } = MessageService.updateReceipt(receiptPayload.message_id, senderId, 'READ');
          if (updated && chat_id) {
        ...
        sendError('INTERNAL_ERROR', err.message || 'Server error processing packet');
      }
    });
